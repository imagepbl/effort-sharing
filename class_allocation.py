...
class allocation(object):
    ...
    def ecpc(self):
        ...
        hist_emissions_timeframe = np.arange(hist_emissions_startyear, 1 + self.start_year_analysis)
        ...
        hist_emissions = self.xr_total.GHG_hist.sel(Time = hist_emissions_timeframe).sum(dim='Time')
    ...
    def save(self):
        #print('- Save')
        xr_total_onlyalloc = self.xr_total.drop_vars(['Population', "CO2_hist", 
                                                      "CO2_globe", "N2O_hist", 
                                                      "CH4_hist", 'GDP', 'GHG_hist', 
                                                      'GHG_globe', "CH4_globe", 
                                                      "N2O_globe", "GHG_hist_all", 
                                                      'GHG_base', 'GHG_ndc', 'Hot_air', 
                                                      'Conditionality', 'Ambition', 
                                                      'Region', 'Budget']).sel(Time=self.analysis_timeframe).astype("float32")
