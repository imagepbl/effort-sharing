...
class allocation():
    ...
    def ecpc(self):
        ...
        hist_emissions_timeframes = [np.arange(startyear, 1 + self.start_year_analysis) for startyear in self.dim_histstartyear]
        past_timelines = [np.arange(startyear, self.start_year_analysis + 1) for startyear in self.dim_histstartyear]
        discount_factors = np.array(self.dim_discountrates)

        for startyear, hist_emissions_timeframe, past_timeline in zip(self.dim_histstartyear, hist_emissions_timeframes, past_timelines):
            ...
            xr_discount = xr.DataArray((1-discount_factors[:, None] / 100) ** discount_period, dims=['Discount_factor', 'Time'], coords={'Discount_factor': discount_factors, 'Time': past_timeline})
            hist_emissions_rt = hist_emissions * xr_discount
            hist_emissions_wt = hist_emissions_rt.sel(Region='EARTH')
            historical_leftover = (share_popt_past * hist_emissions_wt - hist_emissions_rt).sel(Time=np.arange(startyear, 2020 + 1)).sum(dim='Time').sel(Region=self.focus_region)
            ...
            for conv_year in self.dim_convyears:
                ...
                emissions_rightful_at_year = global_emissions_future * population_data.sel(Region=self.focus_region) / population_data.sel(Region='EARTH')
                historical_leftover_updated = historical_leftover - emissions_ecpc + emissions_rightful_at_year.sel(Time=[2021]).sum(dim='Time')
                ...
                for t in range(2100 - self.start_year_analysis):
                    ...
                    if t < max_time_steps - 1:
                        ...
                        emissions_ecpc = Delta_L * sine_values[t] + globe_new * (GF_frac * (1 - (t + 1) / max_time_steps) + pop_frac * ((t + 1) / max_time_steps))
                        historical_leftover_updated = historical_leftover_updated - emissions_ecpc + emissions_rightful_at_year.sel(Time=time_step)
                        es.append(emissions_ecpc.expand_dims({'Time': [time_step]}))
                    elif t == max_time_steps - 1:
                        emissions_ecpc = pop_frac * globe_new * 0.67 + es[-1].sel(Time=time_step - 1) * 0.33
                        es.append(emissions_ecpc.expand_dims({'Time': [time_step]}))
                    else:
                ...
                xr_ecpc_all_list.append(xr_ecpc_alloc.expand_dims({'Historical_startyear': [startyear], 'Convergence_year': [conv_year]}).to_dataset(name="ECPC"))

        xr_ecpc_all = xr.merge(xr_ecpc_all_list)
        # Create the correct order of dimensions
        xr_ecpc_all = xr_ecpc_all.transpose('Discount_factor', 'Historical_startyear', 'Convergence_year',
                                            'NegEmis', 'NonCO2red', 'Temperature', 'Risk', 'Timing', 'Time', 'Scenario')
