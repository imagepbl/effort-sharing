...
class allocation_comb(object):
    ...
    def __init__(self):
        ...
        self.xr_dataread = xr.open_dataset(self.settings['paths']['data']['datadrive'] + "xr_dataread.nc").load().sel(Temperature=[1.6, 2.0],
                                                                                                                      Risk=[0.5, 0.33],
                                                                                                                      NonCO2red=0.5,
                                                                                                                      Timing='Immediate',
                                                                                                                      NegEmis=0.5)
    ...
    def discounting_historical_emissions(self):
        ...
        for hist_emissions_startyear in hist_emissions_startyears:
            for discount_rate in discount_rates:
                hist_emissions = self.xr_dataread.sel(Time = np.arange(hist_emissions_startyear, self.settings['params']['start_year_analysis'])).GHG_hist_excl
    ...
    def ecpc(self):
        ...
        for startyear_i, startyear in enumerate(hist_emissions_startyears):
            ...
            hist_emissions = self.xr_dataread.GHG_hist_excl.sel(Time = hist_emissions_timeframe)
            ...
            for discount_i, discount in enumerate([0, 1.6, 2.0, 2.8]):
                ...
                future_emissions_w = self.xr_dataread.GHG_globe_excl.sel(Time = future_emissions_timeframe).sum(dim='Time')

                total_emissions_w = hist_emissions_w + future_emissions_w

                # Calculating the cumulative population shares for region and world
                cum_pop = self.xr_dataread.Population.sel(Time = self.analysis_timeframe).sum(dim='Time')
                cum_pop_r = cum_pop
                ...
                globalpath = self.xr_dataread.GHG_globe_excl

                emis_2021_i = self.xr_dataread.GHG_hist_excl.sel(Time=self.start_year_analysis)
                emis_2021_w = self.xr_dataread.GHG_hist_excl.sel(Time=self.start_year_analysis, 
                                                            Region='EARTH')
    ...
    def approach1gdp(self):
        ...
        '''
        yearly_netto = self.xr_dataread.GHG_globe_excl.sel(Time=self.all_future_years)
    ...
    def approach2(self):
        ...
        app2_pos_shares = pop2gdp.where(~pop2gdp.Region.isin(["AND", "ATG", "DMA", "GRD", "KIR", "MHL", "FSM", "MCO", "NRU", "PRK", "PLW", "KNA", "SMR", "SYC", "SSD", "TUV",    "COK", "VAT", "NIU", "SOM", "GMB", "LIE", "PSE"]), np.nan)
        app2_pos_shares = app2_pos_shares / app2_pos_shares.sel(Region=self.countries_iso, Time=self.all_future_years).sum(dim='Region')
        app2_pos_shares = app2_pos_shares
        app2_current_debt = self.historical_emissions_discounted.sum(dim='Time')
        app2_current_debt = app2_current_debt.where(~app2_current_debt.Region.isin(["AND", "ATG", "DMA", "GRD", "KIR", "MHL", "FSM", "MCO", "NRU", "PRK", "PLW", "KNA", "SMR", "SYC", "SSD", "TUV",    "COK", "VAT", "NIU", "SOM", "GMB", "LIE", "PSE"]), np.nan)
        app2_nets = []
        app2_negs = []
        app2_poss = []
        for y_i, y in enumerate(self.all_future_years):
            neg = (yearly_neg.sel(Time=y)*(app2_current_debt/app2_current_debt.sel(Region='EARTH'))).drop_vars(['Time'])
    ...
    def approach2_transition(self):
        ...
        self.gf = self.xr_dataread.GHG_hist_excl.sel(Time=self.settings['params']['start_year_analysis']) / self.xr_dataread.GHG_hist_excl.sel(Time=self.settings['params']['start_year_analysis'], Region='EARTH') * yearly_netto
        self.pc = self.xr_dataread.Population / self.xr_dataread.Population.sel(Region='EARTH') * yearly_netto
        gf_f = self.gf / yearly_netto
        pc_f = self.pc / yearly_netto

        self.convergence_moment = 2040
        yearfracs_2 = xr.Dataset(data_vars={"Value": (['Time'], (self.all_future_years-self.settings['params']['start_year_analysis'])/(self.convergence_moment-self.settings['params']['start_year_analysis']))},
                                    coords={"Time": self.all_future_years})
        f2_f = (self.COMB2 / yearly_netto).GHG_hist_excl
        f2c = self.COMB2.GHG_hist_excl.copy()
        self.COMB2t = (f2c.where(f2c.Time > self.convergence_moment, (gf_f*(1-yearfracs_2) + f2_f*yearfracs_2)*yearly_netto)).expand_dims(Convergence_year=[2040])
        ...
        for group_of_choice in ['G20', 'EU', 'G7', 'SIDS', 'LDC', 'Northern America', 'Australasia', 'African Group', 'Umbrella']:
            ...
            group_coord = xr.DataArray(
                [group for country in np.array(self.xr_dataread['Region']) for group in country_to_eu[country]],
                dims=['Region'],
                coords={'Region': [country for country in np.array(self.xr_dataread['Region']) for group in country_to_eu[country]]}
            )
    ...
    def combine(self):
        self.xr_combs = xr.merge([self.COMB1g.to_dataset(name='Approach1_gdp'),
                                  self.COMB1h.to_dataset(name='Approach1_hdi'),
                                  self.COMB2.GHG_hist_excl.to_dataset(name='Approach2'),
    ...
    def get_relation_2030emis_temp(self):
        df_ar6_2 = pd.read_csv(self.settings['paths']['data']['external']+"IPCC/AR6_Scenarios_Database_World_v1.1.csv")
        df_ar6_2 = df_ar6_2[df_ar6_2.Variable.isin(['Emissions|Kyoto Gases',
        ...
        x_fit = x_fit[mask]
        y_fit = y_fit[mask]
        self.coef_ghg_2030 = np.polyfit(x_fit, y_fit, self.settings['params']['polynomial_fit_2030relation'])
    ...
    def determine_tempoutcomes(self):
        ...
        for n in range(self.settings['params']['polynomial_fit_2030relation']+1):#self.coef_ghg_2030[0]*x_fit**5+ self.coef_ghg_2030[1]*x_fit**4+ self.coef_ghg_2030[2]*x_fit**3 +self.coef_ghg_2030[3]*x_fit**2 +self.coef_ghg_2030[4]*x_fit**1 + self.coef_ghg_2030[5]
            if n == 0:
    ...
    def save(self):
        self.xr_combs.to_netcdf(self.settings['paths']['data']['datadrive']+'xr_combinedapproaches.nc', format='NETCDF4')
        self.xr_temps.to_netcdf(self.settings['paths']['data']['datadrive']+'xr_combinedapproaches_temps.nc', format='NETCDF4')