...
class allocation:
    ...
    def __init__(
        ...
        self.rbw = xr.open_dataset(
            self.savepath + "xr_rbw_" + gas + "_" + lulucf + ".nc"
        ).load()
        self.lulucf_indicator = lulucf
        self.gas_indicator = "_" + gas

        # Get dimension lists
        self.dim_histstartyear = self.settings["dimension_ranges"][
            "hist_emissions_startyears"
        ]
    ...
    def gf(self):
        ...
        current_co2_earth = 1e-9 + self.emis_hist.sel(
            Region="EARTH", Time=self.start_year_analysis
        )
    ...
    def pcc(self):
        ...
        def transform_time(time, convyear):
            ...
            during_conv = (fractions["Year"] >= start_conv) & (
                fractions["Year"] < convyear
            )
        ...
        for year in self.dim_convyears:
            ...
            gfdeel.append(
                xr.DataArray(data=ar, dims=dims, coords=coords).to_dataset(name="PCC")
            )
            pcdeel.append(
                xr.DataArray(data=1 - ar, dims=dims, coords=coords).to_dataset(
                    name="PCC"
                )
            )
        ...
        xr_new = (gfdeel_single * self.xr_total.GF + pcdeel_single * self.xr_total.PC)[
            "PCC"
        ]
    ...
    def pcb(self):
        ...
        if self.lulucf_indicator == "incl":
            ...
            emis_start_w = self.xr_total.CO2_hist_excl.sel(
                Time=start_year, Region="EARTH"
            )
        ...
        budget_left = (
            self.emis_fut.where(self.emis_fut > 0, 0)
            .sel(Time=time_range)
            .sum(dim="Time")
            * pop_fraction
        ).sel(Region=focus_region)

        co2_budget_left = (self.xr_total.Budget * pop_fraction).sel(
            Region=focus_region
        ) * 1e3

        budget_without_assumptions_prepeak = path_scaled_0.where(
            path_scaled_0 > 0, 0
        ).sum(dim="Time")
        ...
        if self.lulucf_indicator == "incl":
            ...
            co2_hist = self.xr_total.CO2_hist_excl.sel(
                Region=focus_region, Time=start_year
            )
        time_range = np.arange(start_year, 2101)
        ...
        if self.gas_indicator == "_GHG":
            ...
            xr_comp = xr.DataArray(
                compensation_form, dims=["Time"], coords={"Time": time_range}
            )
        ...
        self.xr_total = self.xr_total.assign(
            PCB=self.ghg_pcb.PCB, PCB_lin=self.ghg_pcb_lin.PCB_lin
        )
    ...
    def ecpc(self):
        ...
        global_emissions_future = self.xr_total.GHG_globe.sel(
            Time=self.analysis_timeframe
        )
        ...
        share_popt_past = self.xr_total.Population / self.xr_total.Population.sel(
            Region="EARTH"
        )
        ...
        for startyear, hist_emissions_timeframe, past_timeline in zip(
            ...
            for conv_year in self.dim_convyears:
                ...
                sine_values = (
                    np.sin(np.arange(1, max_time_steps) / max_time_steps * np.pi) * 3
                )
                ...
                for t in range(2100 - self.start_year_analysis):
                    ...
                    if t < max_time_steps - 1:
                        ...
                        emissions_ecpc = (
                            pop_frac * globe_new * 0.67
                            + es[-1].sel(Time=time_step - 1) * 0.33
                        )
                        es.append(emissions_ecpc.expand_dims({"Time": [time_step]}))
                    else:
    ...
    def ap(self):
        ...
        base_worldsum = self.emis_base.sel(Time=self.analysis_timeframe).sel(
            Region="EARTH"
        )
        rb_part1 = (
        ...
        rb_part2 = (
            self.emis_base.sel(Time=self.analysis_timeframe).sel(
    ...
    def gdr(self):
        ...
        """
        xr_rci = xr.open_dataset(
            self.settings["paths"]["data"]["datadrive"] + "xr_rci.nc"
        ).load()
        ...
        if self.focus_region != "EU":
            ...
            df = pd.read_excel(
                "X:/user/dekkerm/Data/UNFCCC_Parties_Groups_noeu.xlsx",
