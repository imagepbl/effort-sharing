...
class allocation_comb(object):
    ...
    def __init__(self, dataread_file="xr_dataread.nc"):
        ...
        self.all_future_years = np.arange(
    ...
    def ecpc(self):
        ...
        for startyear_i, startyear in enumerate(hist_emissions_startyears):
            hist_emissions_timeframe = np.arange(
                startyear, 1 + self.start_year_analysis
            )
            ...
            for discount_i, discount in enumerate([0, 1.6, 2.0, 2.8]):
                ...
                emis_start_w = self.emis_hist.sel(
                    Time=self.start_year_analysis, Region="EARTH"
                )
                ...
                ecpc = (
                    ...
                    .expand_dims(
                        Discount_factor=[discount], Historical_startyear=[startyear]
                    )
    ...
    def approach1gdp(self):
        ...
        app1_gdp = app1_gdp.where(
            ~app1_gdp.Region.isin(
                ["COK", "VAT", "NIU", "SOM", "GMB", "LIE", "PSE", "MCO", "NRU"]
            ),
            np.nan,
        )
        app1_gdp_neg = (
            app1_gdp
            / app1_gdp.sel(Region="EARTH", Time=self.all_future_years)
            * yearly_neg
        )
        app1_gdp_pos_total = self.xr_ecpc.ECPC.sum(dim="Time") + app1_gdp_neg.sum(
            dim="Time"
        )
        app1_gdp_pos = (
            app1_gdp_pos_total / app1_gdp_pos_total.sel(Region="EARTH") * yearly_pos
        )
    ...
    def approach1hdi(self):
        ...
        app1_hdi = app1_hdi.where(
            ~app1_hdi.Region.isin(
                ["COK", "VAT", "NIU", "SOM", "GMB", "LIE", "PSE", "MCO", "NRU"]
            ),
            np.nan,
        )
        app1_hdi_neg = app1_hdi * yearly_neg
        app1_hdi_pos_total = self.xr_ecpc.ECPC.sum(dim="Time") + app1_hdi_neg.sum(
            dim="Time"
        )
        app1_hdi_pos = (
            app1_hdi_pos_total / app1_hdi_pos_total.sel(Region="EARTH") * yearly_pos
        )
    ...
    def approach2(self):
        ...
        for y_i, y in enumerate(self.all_future_years):
            neg = (
                yearly_neg.sel(Time=y)
                * (app2_current_debt / app2_current_debt.sel(Region="EARTH"))
            ).drop_vars(["Time"])
            pos = (yearly_pos.sel(Time=y) * app2_pos_shares.sel(Time=y)).drop_vars(
                ["Time"]
            )
    ...
    def approach2_transition(self):
        ...
        yearfracs_2 = xr.Dataset(
            data_vars={
                "Value": (
                    ["Time"],
                    (
                        self.all_future_years
                        - self.settings["params"]["start_year_analysis"]
                    )
                    / (
                        self.convergence_moment
                        - self.settings["params"]["start_year_analysis"]
                    ),
                )
            },
        ...
        for group_of_choice in [
            ...
            if group_of_choice == "EU":
                xr_eu = (
                    self.COMB2t[["Value"]].groupby(group_coord).sum()
                )  # skipna=False)
            else:
            ...
            dummy = self.COMB2t.reindex(
                Region=[x for x in list_of_regions if x != group_of_choice]
            )
    ...
    def determine_tempoutcomes_regression(self):
        ...
        percs = self.xr_combs[rules] / self.xr_combs.sel(
        ...
        for n in range(
            ...
            if n == 0:
                xr_temps = (
                    ndc_globalversion**n
                    * self.coef_ghg_2030[
                        self.settings["params"]["polynomial_fit_2030relation"] - n
                    ]
                )
            else:
                xr_temps += (
    ...
    def determine_tempoutcomes_discrete(self):
        ...
        # Now without C1+C2:
        xr_eval = xr_eval_raw.sel(Category=["C1", "C2", "C3", "C6", "C7"]).sum(
            dim="Category"
        )
    ...
    def save(self):
        ...
        self.xr_temps_c12.to_netcdf(
            self.savepath + "xr_combtemps_c12.nc", format="NETCDF4"
        )
