...
class datareading(object):
    ...
    def __init__(self, settings_file=None):
        ...
        # Get dimension lists
        self.dim_temp = (
            np.array(self.settings["dimension_ranges"]["peak_temperature"])
            .astype(float)
            .round(2)
        )
        self.dim_prob = np.array(
            self.settings["dimension_ranges"]["risk_of_exceedance"]
        ).round(2)
        self.dim_negemis = np.array(
            self.settings["dimension_ranges"]["negative_emissions"]
        ).round(2)
        self.dim_nonco2 = np.array(
            self.settings["dimension_ranges"]["non_co2_reduction"]
        ).round(2)
        self.dim_timing = np.array(
            self.settings["dimension_ranges"]["timing_of_mitigation_action"]
        )

        self.time_future = np.arange(
            self.settings["params"]["start_year_analysis"], 2101
        )
        self.time_past = np.arange(
            1850, self.settings["params"]["start_year_analysis"] + 1
        )
        self.savepath = (
    ...
    def read_undata(self):
        ...
        df_pop = pd.read_csv(
            ...
            + "population_HYDE_UNP_Gapminder.csv"
    ...
    def read_hdi(self):
        ...
        for r_i, r in enumerate(self.countries_iso):
            ...
            if len(wh) > 0:
                ...
                wh = np.where(hdi_countries_raw == "Bolivia (Plurinational State of)")[
                    0
                ][0]
                hdi_values[r_i] = hdi_values_raw[wh_i]
            elif r == "COD":
                wh = np.where(
                    hdi_countries_raw == "Congo (Democratic Republic of the)"
                )[0][0]
                hdi_values[r_i] = hdi_values_raw[wh_i]
            elif r == "COG":
                ...
                wh = np.where(
                    hdi_countries_raw == "Korea (Democratic People's Rep. of)"
                )[0][0]
                hdi_values[r_i] = hdi_values_raw[wh_i]
            elif r == "KOR":
                ...
                wh = np.where(hdi_countries_raw == "Lao People's Democratic Republic")[
                    0
                ][0]
                hdi_values[r_i] = hdi_values_raw[wh_i]
            elif r == "FSM":
                wh = np.where(hdi_countries_raw == "Micronesia (Federated States of)")[
                    0
                ][0]
                hdi_values[r_i] = hdi_values_raw[wh_i]
            elif r == "MDA":
                ...
                wh = np.where(hdi_countries_raw == "Saint Vincent and the Grenadines")[
                    0
                ][0]
                hdi_values[r_i] = hdi_values_raw[wh_i]
            elif r == "SWZ":
                ...
                wh = np.where(hdi_countries_raw == "Tanzania (United Republic of)")[0][
                    0
                ]
                hdi_values[r_i] = hdi_values_raw[wh_i]
            elif r == "TUR":
                ...
                wh = np.where(
                    hdi_countries_raw == "Venezuela (Bolivarian Republic of)"
                )[0][0]
                hdi_values[r_i] = hdi_values_raw[wh_i]
            elif r == "VNM":
    ...
    def read_historicalemis_jones(self):
        ...
        xr_primap2 = xr.open_dataset(
            "/home/peter/pbl/effort-sharing/data/input/Guetschow_et_al_2024-PRIMAP-hist_v2.5.1_final_no_rounding_27-Feb-2024.nc"
        )
        df_nwc = pd.read_csv(
            "/home/peter/pbl/effort-sharing/data/input/EMISSIONS_ANNUAL_1830-2022.csv"
        )
        xr_nwc = xr.Dataset.from_dataframe(
            df_nwc.drop(columns=["CNTR_NAME", "Unit"]).set_index(
                ["ISO3", "Gas", "Component", "Year"]
            )
        )
        ...
        xr_nwc["ISO3"] = regs
        xr_nwc_tot = (
            xr_nwc.sel(Gas="CH[4]") * self.settings["params"]["gwp_ch4"] / 1e3
            + xr_nwc.sel(Gas="N[2]*O") * self.settings["params"]["gwp_n2o"] / 1e3
            + xr_nwc.sel(Gas="CO[2]") * 1
        ).drop_vars(["Gas"])
        xr_nwc_co2 = xr_nwc.sel(Gas="CO[2]").drop_vars(["Gas"])
        xr_nwc_ch4 = (
            xr_nwc.sel(Gas="CH[4]").drop_vars(["Gas"])
            * self.settings["params"]["gwp_ch4"]
            / 1e3
        )
        xr_nwc_n2o = (
            xr_nwc.sel(Gas="N[2]*O").drop_vars(["Gas"])
        ...
        self.xr_hist = (
            xr.merge(
                [xr_ghghist, xr_ghgexcl, xr_co2hist, xr_co2excl, xr_ch4hist, xr_n2ohist]
            )
            * 1e3
        )
        ...
        df_edgar = (
            pd.read_excel(
                "/home/peter/pbl/effort-sharing/data/input/EDGARv8.0_FT2022_GHG_booklet_2023.xlsx",
        ...
        self.xr_primap = xr_primap2.rename(
            {
                "area (ISO3)": "Region",
        ...
        df = pd.read_excel(
            "/home/peter/pbl/effort-sharing/data/input/UNFCCC_Parties_Groups_noeu.xlsx",
            sheet_name="Country groups",
        )
        countries_iso = np.array(df["Country ISO Code"])
        group_eu = countries_iso[np.array(df["EU"]) == 1]
        self.xr_hist.GHG_hist.loc[dict(Region="EU")] = self.xr_hist.GHG_hist.sel(
            Region=group_eu
        ).sum("Region")
    ...
    def read_ar6(self):
        ...
        vetting_nans = np.array(
            self.xr_ar6_prevet.ModelScenario[
                ~np.isnan(
                    self.xr_ar6_prevet.Value.sel(Time=2100, Variable="Emissions|CO2")
                )
            ]
        )
            ...
            self.xr_ar6_prevet.ModelScenario[
                np.where(
                    np.abs(
                        ...
                        - self.xr_hist.sel(
                            Region="EARTH", Time=self.recent_increment
                        ).CO2_hist
                    )
                    < 1e4
                )[0]
        ...
        self.ms_immediate = np.array(
            df_ar6_meta[
                df_ar6_meta.Policy_category.isin(["P2", "P2a", "P2b", "P2c"])
            ].ModelScenario
        )
        self.ms_delayed = np.array(
            df_ar6_meta[
                df_ar6_meta.Policy_category.isin(["P3a", "P3b", "P3c"])
            ].ModelScenario
        )
        self.xr_ar6_landuse = (
        ...
        xr_ar6_nozeros = xr_both.where(xr_both > -1e9, np.nan).where(
            xr_both != 0, np.nan
        )
        xr_averages = []
        for i in range(6):
        ...
        df_elevate_bunkers = pd.read_csv(
            "/home/peter/pbl/effort-sharing/data/input/elevate-internal_snapshot_1739887620.csv"
        )[:-1]
        ...
        xr_elevate_bunkers = xr_elevate_bunkers.reindex(
            {"Time": np.arange(2010, 2101, 10)}
        )
        ...
        xr_all.loc[dict(Temperature=1.5)] = xr_all.loc[dict(Temperature=1.7)] - vals * (
            0.2 / 1.3
        )
        xr_all.loc[dict(Temperature=1.6)] = xr_all.loc[dict(Temperature=1.7)] - vals * (
            0.1 / 1.3
        )
        xr_all.loc[dict(Temperature=4.0)] = xr_all.loc[dict(Temperature=3.0)] + vals * (
            1.0 / 1.3
        )
        xr_all.loc[dict(Temperature=4.5)] = xr_all.loc[dict(Temperature=3.0)] + vals * (
            1.5 / 1.3
        )
        ...
        xr_all = xr_all.assign_coords(
            Category=["C1", "C1+C2", "C2", "C3", "C6", "C7", "C8"]
        )
    ...
    def relation_budget_nonco2(self):
        ...
        df_nonco2 = pd.read_excel(
            "/home/peter/pbl/effort-sharing/data/input/NonCO2.xlsx", sheet_name="Sheet1"
        )
        ...
        xr_nonco2effects = xr_nonco2effects.reindex(
            NonCO2red=np.arange(0, 0.8001, 0.01).round(2)
        )
        xr_nonco2effects = xr_nonco2effects.interpolate_na(
            dim="NonCO2red", method="linear"
        )
        xr_nonco2effects = xr_nonco2effects.reindex(
            Temperature=list(self.dim_temp) + [2.5]
        )
        xr_nonco2effects = xr_nonco2effects.interpolate_na(
            dim="Temperature", method="linear"
        )
    ...
    def determine_global_nonco2_trajectories(self):
        ...
        xr_ch4_raw = (
            self.xr_ar6.sel(Variable="Emissions|CH4")
            * self.settings["params"]["gwp_ch4"]
        )
        xr_n2o_raw = (
            self.xr_ar6.sel(Variable="Emissions|N2O")
            * self.settings["params"]["gwp_n2o"]
            / 1e3
        )
        n2o_start = (
        ...
        xr_nonco2_raw_start = xr_nonco2_raw.sel(
        ...
        def ms_temp(T):
            peaktemp = self.xr_ar6.sel(
                Variable="AR6 climate diagnostics|Surface Temperature (GSAT)|MAGICCv7.5.3|50.0th Percentile"
            ).Value.max(dim="Time")
            return self.xr_ar6.ModelScenario[
                np.where((peaktemp < T + 0.05) & (peaktemp > T - 0.05))[0]
            ]
        ...
        def rescale(traj):
            offset = (
                traj.sel(Time=self.settings["params"]["start_year_analysis"])
                - tot_start
            )
        ...
        xr_reductions = (
            xr_nonco2_raw.sel(Time=2040) - xr_nonco2_raw_start
        ) / xr_nonco2_raw_start
            ...
            if len(ms) == 0:
                ...
                for n_i, n in enumerate(self.dim_nonco2):
                    ...
                    traj2 = (
                        -(1 - xr_comp)
                        * (red_traj - red)
                        * xr_nonco2_raw_start.mean().Value
                        + trajectory_mean
        ...
        self.xr_traj_nonco2 = self.xr_traj_nonco2.reindex(
            {"Temperature": [1.5, 1.8, 2.1, 2.4]}
        )
        self.xr_traj_nonco2 = self.xr_traj_nonco2.reindex(
    ...
    def determine_global_budgets(self):
        ...
        df_budgets = pd.read_csv(
            "/home/peter/pbl/effort-sharing/data/input/update_MAGICC_and_scenarios-budget.csv"
        )  # Now without the warming update in Forster, to link to IPCC AR6
        df_budgets = df_budgets[
            ["dT_targets", "0.1", "0.17", "0.33", "0.5", "0.66", "0.83", "0.9"]
        ]
        dummy = df_budgets.melt(
            id_vars=["dT_targets"], var_name="Probability", value_name="Budget"
        )
        ...
        if self.settings["params"]["start_year_analysis"] == 2020:
            ...
            for year in np.arange(2020, self.settings["params"]["start_year_analysis"]):
                budgets -= (
                    float(self.xr_hist.sel(Region="EARTH", Time=year).CO2_hist) / 1e3
                )
        elif self.settings["params"]["start_year_analysis"] < 2020:
            budgets = dummy["Budget"]
        ...
        def ms_temp(T):
            ...
            return self.xr_ar6.ModelScenario[
                np.where((peaktemp < T + 0.1) & (peaktemp > T - 0.1))[0]
            ]
        ...
        for t_i, t in enumerate(self.dim_temp):
            ...
            bunker_subtraction += [
                3.3 / 100
            ]  # Assuming bunker emissions keep constant (3.3% of global emissions) - https://www.pbl.nl/sites/default/files/downloads/pbl-2020-analysing-international-shipping-and-aviation-emissions-projections_4076.pdf
        ...
        Blist = (
            np.zeros(
                shape=(len(self.dim_temp), len(self.dim_prob), len(self.dim_nonco2))
            )
            + np.nan
        )
        ...
        for p_i, p in enumerate(self.dim_prob):
            a, b = np.polyfit(
                xr_bud_co2.Temperature,
                xr_bud_co2.sel(Probability=np.round(p, 2)).Budget,
                1,
            )
            for t_i, t in enumerate(self.dim_temp):
                median_budget = (a * t + b) * (
                    1 - bunker_subtraction[t_i]
                )  # relation may slightly deviate from Forster
                for n_i, n in enumerate(self.dim_nonco2):
                    nonco2_reduction_given_quantile = np.array(
                        (
                            (
                                (
                                    self.xr_traj_nonco2.sel(
                                        NonCO2red=n, Time=2040, Temperature=t
                                    )
                                    - tot_2020
                                )
                                / tot_2020
                    ...
                    nonco2_reduction_given_quantile = nonco2_reduction_given_quantile[
                        ~np.isnan(nonco2_reduction_given_quantile)
                    ]
                    nonco2_reduction_given_quantile[
                        nonco2_reduction_given_quantile > 0
                    ] = 0
                    nonco2_reduction_given_quantile[
                        nonco2_reduction_given_quantile < -0.8
                    ] = -0.8
                    nonco2effect = self.xr_nonco2effects.sel(
                        Temperature=t,
                        NonCO2red=-nonco2_reduction_given_quantile.round(2),
                    ).EffectOnRCB  # Effect on RCB based on how different non-CO2 paths are than they are in Forster
    ...
    def determine_global_co2_trajectories(self):
            ...
            compensation_form = np.array(
                list(
                    np.linspace(
                        ...
                        len(
                            np.arange(
                                self.settings["params"]["start_year_analysis"], hy
                            )
                        ),
                    )
                )
        ...
        def budget_harm(nz):
            return xr_comp / np.sum(
                xr_comp.sel(
                    Time=np.arange(self.settings["params"]["start_year_analysis"], nz)
                )
            )
        ...
        co2_start = (
            xr_scen2_use.sel(Time=self.settings["params"]["start_year_analysis"]) / 1e3
        )
        ...
        difyears = 2020 + 1 - self.settings["params"]["start_year_analysis"]
        if difyears > 0:
            emis_all_adapt = emis_all.assign_coords(
                {"Time": emis_all.Time - (difyears - 1)}
            ).reindex(
                {
                    "Time": np.arange(
                        self.settings["params"]["start_year_analysis"], 2101
            ...
            for t in np.arange(0, difyears):
                ...
                dv = dv.where(dv < 0, 0)
                emis_all_adapt.Value.loc[{"Time": 2101 - difyears + t}] = (
                    dv + emis_all_adapt.Value.sel(Time=2101 - difyears + t - 1)
                )
        ...
        def ms_temp(T):
            ...
            return self.xr_ar6.ModelScenario[
                np.where((peaktemp < dt + T + 0.0) & (peaktemp > dt + T - 0.3))[0]
            ]  # This is the core functional form. Temp ranges are chosen based on fitting with IPCC WGIII pathways from C1 and C3 (SYR AR6, table 3.1)
        ...
        for temp_i, temp in enumerate(self.dim_temp):
            ...
            for timing_i, timing in enumerate(self.dim_timing):
                ...
                surplus_factor2 = np.convolve(
                    surplus_factor, np.ones(3) / 3, mode="valid"
                )
                surplus_factor[1:-1] = surplus_factor2

                for neg_i, neg in enumerate(self.dim_negemis):
                    xset = emis_all.sel(ModelScenario=ms2) - surplus_factor * (
                        neg - 0.5
                    )
                    pathways_neg = xr_neg.sel(ModelScenario=ms1).quantile(
                        neg, dim="ModelScenario"
                    )
                    pathways_data["CO2_neg_globe"][neg_i, temp_i, :] = np.array(
                        pathways_neg.Value
                    )
                    for risk_i, risk in enumerate(self.dim_prob):
                        for nonco2_i, nonco2 in enumerate(self.dim_nonco2):
                            ...
                            if len(all_pathways) > 0:
                                ...
                                pathway_sep = np.convolve(
                                    pathway, np.ones(3) / 3, mode="valid"
                                )
                                ...
                                pathway_final2 = np.array(
                                    (1e3 * (pathway_final + factor * budget_harm(nz)))
                                    / 1e3
                                )
        ...
        self.xr_traj_ghg = (
            self.xr_traj_co2.CO2_globe + self.xr_traj_nonco2.NonCO2_globe
        ).to_dataset(name="GHG_globe")
        ...
        landuse_ghg_hist = (
            self.xr_hist.sel(Region="EARTH").GHG_hist
            - self.xr_hist.sel(Region="EARTH").GHG_hist_excl
        )
        landuse_co2_hist = (
            self.xr_hist.sel(Region="EARTH").CO2_hist
            - self.xr_hist.sel(Region="EARTH").CO2_hist_excl
        )
        ...
        self.xr_traj_ghg_excl = (
            self.xr_traj_ghg.GHG_globe - self.landuse_ghg_corr
        ).to_dataset(name="GHG_globe_excl")
        self.xr_traj_co2_excl = (
            self.xr_traj_co2.CO2_globe - self.landuse_co2_corr
        ).to_dataset(name="CO2_globe_excl")
    ...
    def read_baseline(self):
        ...
        for i in range(
            ...
            df_base = pd.read_excel(
                self.settings["paths"]["data"]["baseline"]
                + "SSP"
                + str(i + 1)
                + ".xlsx",
                sheet_name="Sheet1",
        ...
        xr_base = xr_base.reindex(
            Time=np.arange(self.settings["params"]["start_year_analysis"], 2101)
        )
        for year in np.arange(self.settings["params"]["start_year_analysis"], 2021):
            xr_base.CO2_base_excl.loc[dict(Time=year, Region=self.countries_iso)] = (
                self.xr_hist.sel(Time=year, Region=self.countries_iso).CO2_hist_excl
            )

        # Harmonize emissions from historical values to baseline emissions
        diffrac = self.xr_hist.CO2_hist_excl.sel(
            Time=self.settings["params"]["start_year_analysis"]
        ) / xr_base.CO2_base_excl.sel(
            Time=self.settings["params"]["start_year_analysis"]
        )
        ...
        xr_base = xr_base.assign(
            CO2_base_incl=xr_base.CO2_base_excl * fraction_startyear_co2_incl
        )
        ...
        xr_base = xr_base.assign(
            GHG_base_excl=xr_base.CO2_base_excl * fraction_startyear_ghg_excl
        )
        ...
        xr_base = xr_base.assign(
            GHG_base_incl=xr_base.CO2_base_excl * fraction_startyear_ghg_incl
        )
        ...
        for year in np.arange(self.settings["params"]["start_year_analysis"], 2021):
            xr_base.GHG_base_excl.loc[dict(Time=year, Region=self.countries_iso)] = (
                self.xr_hist.sel(Time=year, Region=self.countries_iso).GHG_hist_excl
            )
            xr_base.CO2_base_incl.loc[dict(Time=year, Region=self.countries_iso)] = (
                self.xr_hist.sel(Time=year, Region=self.countries_iso).CO2_hist
            )
            xr_base.GHG_base_incl.loc[dict(Time=year, Region=self.countries_iso)] = (
                self.xr_hist.sel(Time=year, Region=self.countries_iso).GHG_hist
            )
    ...
    def merge_xr(self):
        ...
        self.xr_total = xr_total.interpolate_na(dim="Time", method="linear").drop_vars(
            ["Variable"]
        )
    ...
    def add_country_groups(self):
        ...
        path_ctygroups = (
            "/home/peter/pbl/effort-sharing/data/input/"
        ...
        self.xr_total["GHG_base_incl"][
            np.where(self.xr_total.Region == "EU")[0], np.array([3, 4])
        ] = np.nan  # SSP4, 5 are empty for Europe!
        self.xr_total["CO2_base_incl"][
            np.where(self.xr_total.Region == "EU")[0], np.array([3, 4])
        ] = np.nan  # SSP4, 5 are empty for Europe!
        self.xr_total["GHG_base_excl"][
            np.where(self.xr_total.Region == "EU")[0], np.array([3, 4])
        ] = np.nan  # SSP4, 5 are empty for Europe!
        self.xr_total["CO2_base_excl"][
            np.where(self.xr_total.Region == "EU")[0], np.array([3, 4])
        ] = np.nan  # SSP4, 5 are empty for Europe!
    ...
    def save(self):
        ...
        xr_normal = self.xr_total.sel(
            Temperature=np.array(
                self.settings["dimension_ranges"]["peak_temperature_saved"]
            )
            .astype(float)
            .round(2)
        )
        ...
        for gas in ["CO2", "GHG"]:
            for lulucf_i, lulucf in enumerate(["incl", "excl"]):
                ...
                r1_nom = xrt.GDP.sel(Region="EARTH") / xrt.Population.sel(
                    Region="EARTH"
                )
    ...
    def country_specific_datareaders(self):
        ...
        xr_dataread_nld = (
            xr.open_dataset(self.savepath + "xr_dataread.nc").load().copy()
        )
        dutch_time = np.array(
            [
                1990,
        ...
        dutch_time_interp = np.arange(
            1990, self.settings["params"]["start_year_analysis"] + 1
        )
        dutch_ghg_interp = np.interp(dutch_time_interp, dutch_time, dutch_ghg)
        fraction_1990 = float(
            dutch_ghg[0] / self.xr_total.GHG_hist.sel(Region="NLD", Time=1990)
        )
        pre_1990_raw = (
            np.array(
                self.xr_total.GHG_hist.sel(Region="NLD", Time=np.arange(1850, 1990))
            )
            * fraction_1990
        )
        ...
        for t_i, t in enumerate(self.time_past):
            xr_dataread_nld.GHG_hist.loc[dict(Time=t, Region="NLD")] = total_ghg_nld[
                t_i
            ]

        xr_dataread_nld.CO2_base_incl.loc[dict(Region="NLD", Time=self.time_future)] = (
            xr_dataread_nld.CO2_base_incl.sel(Region="NLD", Time=self.time_future)
            / fractions[-1]
        )
        xr_dataread_nld.CO2_base_excl.loc[dict(Region="NLD", Time=self.time_future)] = (
            xr_dataread_nld.CO2_base_excl.sel(Region="NLD", Time=self.time_future)
            / fractions[-1]
        )
        xr_dataread_nld.GHG_base_incl.loc[dict(Region="NLD", Time=self.time_future)] = (
            xr_dataread_nld.GHG_base_incl.sel(Region="NLD", Time=self.time_future)
            / fractions[-1]
        )
        xr_dataread_nld.GHG_base_excl.loc[dict(Region="NLD", Time=self.time_future)] = (
            xr_dataread_nld.GHG_base_excl.sel(Region="NLD", Time=self.time_future)
            / fractions[-1]
        )
        ...
        xr_dataread_nld.CO2_hist_excl.loc[dict(Region="NLD", Time=self.time_past)] = (
            xr_dataread_nld.CO2_hist_excl.sel(Region="NLD", Time=self.time_past)
            / fractions
        )
        xr_dataread_nld.GHG_hist_excl.loc[dict(Region="NLD", Time=self.time_past)] = (
            xr_dataread_nld.GHG_hist_excl.sel(Region="NLD", Time=self.time_past)
            / fractions
        )
        xr_dataread_nld.sel(
            Temperature=np.array(
        ...
        ghg_axis = np.array(
            self.xr_primap.sel(
                Scenario="HISTCR", Region="NOR", time=time_axis, Category="M.0.EL"
            )["KYOTOGHG (AR6GWP100)"]
        )
        ...
        fraction_minyear = float(
            ghg_axis[0]
            / self.xr_total.GHG_hist_excl.sel(Region="NOR", Time=np.min(time_axis))
        )
        pre_minyear_raw = (
            np.array(
                self.xr_total.GHG_hist_excl.sel(
                    Region="NOR", Time=np.arange(1850, np.min(time_axis))
                )
            )
            * fraction_minyear
        )
        ...
        fractions = np.array(
            xr_dataread_nor.GHG_hist_excl.sel(Region="NOR", Time=self.time_past)
            / total_ghg_nor
        )
        for t_i, t in enumerate(self.time_past):
            xr_dataread_nor.GHG_hist_excl.loc[dict(Time=t, Region="NOR")] = (
                total_ghg_nor[t_i]
            )

        xr_dataread_nor.CO2_base_incl.loc[dict(Region="NOR", Time=self.time_future)] = (
            xr_dataread_nor.CO2_base_incl.sel(Region="NOR", Time=self.time_future)
            / fractions[-1]
        )
        xr_dataread_nor.CO2_base_excl.loc[dict(Region="NOR", Time=self.time_future)] = (
            xr_dataread_nor.CO2_base_excl.sel(Region="NOR", Time=self.time_future)
            / fractions[-1]
        )
        xr_dataread_nor.GHG_base_incl.loc[dict(Region="NOR", Time=self.time_future)] = (
            xr_dataread_nor.GHG_base_incl.sel(Region="NOR", Time=self.time_future)
            / fractions[-1]
        )
        xr_dataread_nor.GHG_base_excl.loc[dict(Region="NOR", Time=self.time_future)] = (
            xr_dataread_nor.GHG_base_excl.sel(Region="NOR", Time=self.time_future)
            / fractions[-1]
        )
        ...
        xr_dataread_nor.CO2_hist_excl.loc[dict(Region="NOR", Time=self.time_past)] = (
            xr_dataread_nor.CO2_hist_excl.sel(Region="NOR", Time=self.time_past)
            / fractions
        )
